...
Begin VB.Form Main 
   ...
   Begin VB.Label Label 
      Caption         =   "PoUpdate: Busche Alabama"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   13.5
         Charset         =   0
...
SourceConn.Open = "Provider=SQLOLEDB.1;Data Source=Hartselle-SQL-1; Initial Catalog=Cribmaster;user id = 'sa';password='buschecnc1'"
DestinationConn.Open = "Provider=SQLOLEDB.1;Data Source=Hartselle-SQL; Initial Catalog=m2mdata01;user id = 'sa';password='buschecnc1'"
...
'*******************CHECK IF ALL PO CATEGORIES HAVE BEEN SELECTED FOR EACH PO ITEM & THE RECORDS ARE NOT LOCKED****************
  ...
  While Not ItemSourceRecordset.EOF
      If IsNull(ItemSourceRecordset.Fields("UDFHSPOCAT")) Then
        Err.Description = "Error: No Alabama PO Category for Po# " + CStr(SourceRecordset.Fields("PONUMBER")) + " Item# " + CStr(ItemSourceRecordset.Fields("Item"))
        GoTo ErrorHandler
    End If
...
'*******************CHECK IF PO HAS A VALID VENDOR IN CRIBMASTER****************
  ...
  If (IsNull(M2MVendor)) Then
    Err.Description = "Error: No Alabama M2M Vendor found for Cribmaster Vendor# " + CStr(SourceRecordset.Fields("Vendor")) + " found for PO# " + PONumber
    GoTo ErrorHandler
  End If
...
'  CompanySourceRecordset.Open "SELECT * FROM [APVEND],[SYADDR] WHERE [APVEND].FVENDNO = [SYADDR].FCALIASKEY AND fvendno = '" + GetVendorNumber.Fields("UDFM2MVENDORNUMBER") + "' AND [SYADDR].FCALIAS = 'APVEND'", DestinationConn
  If CompanySourceRecordset.RecordCount < 1 Then
     Err.Description = "Error: Alabama Vendor #" + CStr(GetVendorNumber.Fields("UDFM2MVENDORNUMBER")) + " not in M2M for PO# " + CStr(SourceRecordset.Fields("PONumber"))
     GoTo ErrorHandler
  End If
'*******************GET STANDARD TERMS FROM M2M VENDOOR PAGE***************
  ...
  If Not SkipPO Then
   ...
   DestinationRecordset.Fields("fndbrmod") = 0
    ...
    DestinationRecordset.Fields("fcshcompan") = "BUSCHE ALABAMA"
    DestinationRecordset.Fields("fcshcity") = "HARTSELLE"
    DestinationRecordset.Fields("fcshstate") = "AL"
    DestinationRecordset.Fields("fcshzip") = "35640"
    DestinationRecordset.Fields("fcshcountr") = "USA"
    DestinationRecordset.Fields("fcshphone") = "2567730137"
    DestinationRecordset.Fields("fcshfax") = "2567512344"
    DestinationRecordset.Fields("fmshstreet") = "1641 THOMPSON ROAD"
